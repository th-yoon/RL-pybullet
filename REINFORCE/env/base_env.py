# This is a copy from pybullet_envs.env_bases
...
class PID:
    ...
    def update(self, current_angle, desired_angle):
        # u(t) = k_p * e(t) + k_i * \int_{0}^{t} e(\tau)d\tau + k_d * d(e(t)) / dt
        # U(s) = k_p + k_i / s + k_d * s
        error = desired_angle - current_angle
        self.p = error
        self.i += error * self.tau
        self.d = (error - self.error_prev) * self.tau
        self.output = self.kp * self.p + self.ki * self.i + self.kd * self.d
        self.error_prev = error
        return self.output

    def get_action(self, u):
        # the smaller the u, the smaller the torque it gets 
        # but 0 error is not what we want
        return u 
