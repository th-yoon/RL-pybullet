...
class Policy(torch.nn.Module):
    ...
    def __init__(self, observation_space_shape, action_space_shape, is_continuous):
        ...
        self.affine1 = torch.nn.Linear(self.observation_space_shape, 128)
        self.affine2 = torch.nn.Linear(128, 256)
        self.affine3 = torch.nn.Linear(256, 128)
        if is_continuous:
            self.value_interval = 3 if self.action_space_shape == 1 else 21 
            self.affine4 = torch.nn.Linear(128, self.action_space_shape * self.value_interval)
        else:
