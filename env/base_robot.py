...
class XmlBasedRobot:
    ...
    def addToScene(self, bullet_client, bodies):
        ...
        for i in range(len(bodies)):
            # no joint means there is no other links on that body
            if self._p.getNumJoints(bodies[i]) == 0:
                ...
                #print('no joint, self.robot_name:', self.robot_name)
            ...
            for j in range(self._p.getNumJoints(bodies[i])):
                ...
                #print('base_robot, addToScene, self.robot_name: ', self.robot_name)
    ...
    def reset(self, bullet_client):
        ...
        if (self.doneLoading == 0):
            ...
            if self.self_collision:
                ...
                self.parts, self.jdict, self.ordered_joints, self.robot_body = self.addToScene(
                    self._p, self.objects)
        # defined in the specific robot implementation
...
class Joint:
    ...
    def __init__(self, bullet_client, joint_name, bodies, bodyIndex, jointIndex):
        ...
        jointInfo = self._p.getJointInfo(
