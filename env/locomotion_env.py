# This is mostly from pybullet_envs.gym_locomotion_envs

from .scene_stadium import SinglePlayerStadiumScene
from .base_env import MJCFBaseBulletEnv, PID
import numpy as np
import pybullet
from .half_cheetah import HalfCheetah
from .ant import Ant
from .ant_pentapod import AntPentapod
...
class WalkerBaseBulletEnv(MJCFBaseBulletEnv):
    ...
    def step(self, action_probs, is_pid=False):
        if not self.scene.multiplayer:  # if multiplayer, action first applied to all robots, then global step() called, then _step() for all robots with the same actions
            action = action_probs
            if is_pid:
                u = self.pid.update(
                    current_angle=self.get_motor_angles(), desired_angle=np.pi * (action))
                action = self.pid.get_action(u)
            self.robot.apply_action(action, is_pid=is_pid) if is_pid else self.robot.apply_action(action)
        ...
        # update the potential after applying action
        ...
        electricity_cost = self.electricity_cost * float(np.abs(action_probs * self.robot.joint_speeds).mean(
        ))  # let's assume we have DC motor with controller, and reverse current braking
        electricity_cost += self.stall_torque_cost * \
            float(np.square(action_probs).mean())

        # robot.joints_at_limit represents position, scaled to -1..+1 between limits
        ...
        self.rewards = [
            ...
            #self._alive, progress, joints_at_limit_cost, feet_collision_cost
        ]
        if (debugmode):
            print("rewards=")
            print(self.rewards)
        ...
        self.HUD(state, action_probs, done)
    ...
    def get_motor_angles(self):
        return torch.tensor([self.ordered_joints[i].get_position() for i in range(len(self.ordered_joints))])
...
class AntPentapodBulletEnv(WalkerBaseBulletEnv):

    def __init__(self, render=False):
        self.robot = AntPentapod()
        self.name = 'AntPentapod'
        WalkerBaseBulletEnv.__init__(self, self.robot, render)
